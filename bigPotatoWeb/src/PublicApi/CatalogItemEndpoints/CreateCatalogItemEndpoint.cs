...
public class CreateCatalogItemEndpoint : IEndpoint<IResult, CreateCatalogItemRequest>
{
    ...
    public async Task<IResult> HandleAsync(CreateCatalogItemRequest request)
    {
        ...
        var newItem = new CatalogItem(request.CatalogTypeId, request.CatalogBrandId, request.Description, request.Name, request.Price, request.PictureUri, request.ETA);
        ...
        if (newItem.Id != 0)
        {
            ...
            //  pointed out by the community. More info in this issue: https://github.com/dotnet-architecture/bigPotatoWeb/issues/537 
